...
const GeneralModaler = (props: GeneralModalerProps) => {
  ...
  return loaders?.map((loader, index) => {
    ...
    return (
      ...
      >
        <DialogContent
          className={generalModalVariants({
            type: detail.type,
            size: detail?.size,
            className: cn(detail?.generalProps?.className),
          })}
        >
          <DialogHeader
            className={cn(
              "flex flex-row flex-wrap items-center justify-start",
              detail.centerTitle ? "justify-center" : "",
            )}
          >
            ...
            <DialogTitle dangerouslySetInnerHTML={{ __html: detail?.title }} className="mx-auto" />
            {detail?.description ? (
          ...
          {actions.length ? (
            <DialogFooter
              className={cn(
                "mt-auto flex gap-1 px-1",
                { start: "sm:justify-start", center: "sm:justify-center", end: "sm:justify-end" }[
                  (detail.actionProps?.align ?? "end") as "start" | "center" | "end"
                ],
              )}
            >
